 ...
 *
 * Verbose logging is on by default in development and can be toggled at
 * runtime from the console:  window.__nexoraLocationDebug = true|false
 */
...
export class Logger {
  ...
  /** Structured single-line log for one raw GPS reading. */
  ...
  private write(level: LogLevel, message: string, data?: unknown) {
    this.trail.push({ at: Date.now(), level, message, data });
    ...
    if (LEVEL_WEIGHT[level] < LEVEL_WEIGHT.warn && !debugFlag()) return;
