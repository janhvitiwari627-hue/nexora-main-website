...
begin
  ...
  if p_webhook_secret is not null and length(p_webhook_secret) > 0 then
    -- TODO: replace with cryptographically secure verify once secret storage
    -- is wired (e.g., vault / Edge Function env).
    verified := true;
  else
    -- When no secret is configured, mark unverified but still record.
    verified := false;
  end if;
...
begin
  ...
  -- Update projections here (e.g., booking payment status, wallet credit).
  -- All mutations are inside this transaction so rollback is atomic.
...
-- ---------------------------------------------------------------------------
-- The following policies must be applied to the Supabase Storage buckets
-- via the Dashboard or CLI. They are included here as comments so the
-- contract is version-controlled and auditable.
--
-- Bucket: salon-media
...
--
-- Example SQL (run via supabase storage policies or dashboard):
--
-- create policy "salon_media_public_read"
--   on storage.objects for select
--   using (bucket_id = 'salon-media' and name like 'salon/%/public/%');
--
-- create policy "salon_media_owner_write"
--   on storage.objects for insert
--   with check (
--     bucket_id = 'salon-media'
--     and name like 'salon/' || auth.uid() || '/owner/%'
--     and (storage.extension(name) in ('jpg','jpeg','png','webp','mp4'))
--   );
--
-- create policy "identity_docs_service_only"
--   on storage.objects for all
--   using (bucket_id = 'identity-documents' and auth.role() = 'service_role');

-- ---------------------------------------------------------------------------
-- 10. Additional RLS policies for tables created in earlier phases
-- ---------------------------------------------------------------------------

-- growth_partners: partners read own row; no direct writes.
do $x$
begin
...
-- organization_members: owners read their own memberships.
do $x$
begin
...
-- salon_setup_proposals: owner can read proposals for their salon;
-- growth partner can read proposals they submitted.
do $x$
begin
...
-- shop_attributions: partner reads their own attributions.
do $x$
begin
...
-- shop_onboarding_applications: partner reads their own applications.
do $x$
begin
...
-- notifications: user reads only their own notifications.
do $x$
begin
...
declare
  ...
  rpc_count integer;
begin
  -- Check 1: every known table has RLS enabled
  ...
  -- Check 2: audit_events table exists and is immutable
  ...
  -- Check 3: payment_webhook_events table exists and is immutable
