...
const buttonSources = [
  ...
  "integration-packages/template-app/back-to-main-website.patch",
];
...
test("the return control is mounted once in each checked-in global app shell/header", () => {
  ...
  const templatePatch = read("integration-packages/template-app/back-to-main-website.patch");
  assert.match(templatePatch, /src\/components\/TopBar\.tsx/);
});
