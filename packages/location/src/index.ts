 ...
 *
 * Live position comes only from navigator.geolocation.watchPosition(). The
 * same authenticated user's last real GPS fix is privately saved in Supabase
 * project qwaehqsmodekbgvnaavz and may be reused as an explicitly saved/stale
 * fallback. Business locations are a separate approval-gated dataset.
 */

export * from "./types";
export {
  LOCATION_SUPABASE_PROJECT_REF,
  LOCATION_SUPABASE_URL,
  assertSharedLocationProject,
} from "./config";
